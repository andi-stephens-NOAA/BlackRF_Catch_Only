...
Exec_catch1 =  read.csv('./txt_files/Exec_catch_for_figs1.csv')
  
# Assign column names
colnames(Exec_catch1) = c('Year',
                         'Fleet 1',
                         'Fleet 2',
                         'Fleet 3',
                         'Fleet 4',
...
Exec_region1_catch = Exec_catch1[,c(1:2)]
Exec_region2_catch = Exec_catch1[,c(1,3,4)]
Exec_region3_catch = Exec_catch1[,c(1,5,6)]
...
Exec_catch.table1 = xtable(Exec_catch_summary1, 
...
# Read in executive summary catches table
Exec_catch_summary2 = read.csv('./txt_files/Exec_catch_summary2.csv')

# Assign column names as they should appear in the table; change the alignment 
# to match number of columns +1
colnames(Exec_catch_summary2) = c('Year', 
                                  'Landings 1',
                                  'Landings 2',
                                  'Landings 3',
                                  'Landings 4', 
...
# Make executive summary catch xtable
Exec_catch.table2 = xtable(Exec_catch_summary2, 
                           caption = c(paste('Recent ',spp,' landings (mt) by 
                                            fleet.', sep='')), 
                           label='tab:Exec_catch')
...
# Read in executive summary catches table
Exec_catch_summary3 = read.csv('./txt_files/Exec_catch_summary3.csv')

# Assign column names as they should appear in the table; change the alignment 
# to match number of columns +1
colnames(Exec_catch_summary3) = c('Year', 
                                  'Landings 1',
                                  'Landings 2',
                                  'Landings 3',
                                  'Landings 4', 
...
# Make executive summary catch xtable
Exec_catch.table3 = xtable(Exec_catch_summary3, 
                           caption = c(paste('Recent ',spp,' landings (mt) by 
                                            fleet.', sep='')), 
                           label='tab:Exec_catch')
...
mngmnt1 = read.csv('./txt_files/Exec_mngmt_performance1.csv')

colnames(mngmnt1) = c('Year',
...
mngmnt2 = read.csv('./txt_files/Exec_mngmt_performance2.csv')

colnames(mngmnt2) = c('Year',
                      'OFL (mt; ABC prior to 2011)',  
                      'ABC (mt)', 
                      'ACL (mt; OY prior to 2011)', 
                      'Estimated total catch (mt)')
...
mngmnt3 = read.csv('./txt_files/Exec_mngmt_performance3.csv')

colnames(mngmnt3) = c('Year',
                      'OFL (mt; ABC prior to 2011)',  
                      'ABC (mt)', 
                      'ACL (mt; OY prior to 2011)', 
                      'Estimated total catch (mt)')
...
# Create the table
      OFL.table1 = xtable(OFL, caption=c('Projections of potential OFL (mt) for 
...
# For 2 models:
      if (n_models > 1) {
        ...
        OFL = as.data.frame(OFL_mod2)
        OFL$Total = rowSums(OFL)
        OFL$Year=seq(Project_firstyr,Project_lastyr-1,1)
        OFL$Year = as.factor(OFL$Year)
        OFL = OFL[,c(2,1)]
        colnames(OFL) = c('Year','OFL') 
        
        # Create the table
        OFL.table2 = xtable(OFL, caption=c('Projections of potential OFL (mt) for each model, using the base model forecast.'),
                           label = 'tab:OFL_projection')     
}           
...
if (n_models == 3) {
      ...
      #Turn into a dataframe and get the total
      #  = as.data.frame(cbind(OFL_mod1, OFL_mod2, OFL_mod3))
      OFL = data.frame(OFL_mod3)
      OFL$Total = rowSums(OFL)
      OFL$Year=seq(Project_firstyr,Project_lastyr-1,1)
      OFL$Year = as.factor(OFL$Year)
      OFL = OFL[,c(2,1)]
      ...
      # Create the table
      OFL.table3 = xtable(OFL, caption=c('Projections of potential OFL (mt) for 
                                        each model, using the base model forecast.'),
                         label = 'tab:OFL_projection')     
...
mngmt1 = read.csv('./txt_files/Exec_basemodel_summary1.csv')
mngmt1 = mngmt1[,-1]
...
if (n_models >= 2) {
  ...
  # Read in the management table
  mngmt2 = read.csv('./txt_files/Exec_basemodel_summary2.csv')
  mngmt2 = mngmt2[,-1]
...
if (n_models == 3) {
  # Read in the management table
  mngmt3 = read.csv('./txt_files/Exec_basemodel_summary3.csv')
  mngmt3 = mngmt3[,-1]
...
#   # Bind data from all three models together
  base_summary1 = as.data.frame(cbind(mngmt1,mod1_summary))
  
  cat("Got 1\n")

  # Transpose the dataframe to create the table and create data labels  
  base_summary1 = as.data.frame(t(base_summary1))
  base_summary1$names=c('Landings (mt)',
                       'Total Est. Catch (mt)',
                       'OFL (mt)', 
                       'ACL (mt)',
  ...
  base_summary1 = base_summary1[,c(ncol(base_summary1),1:(ncol(base_summary1)-1))]
  colnames(base_summary1) = c('Quantity',seq(FirstYR+1,LastYR+1))
  
  # # Create the table``
  base_summary.table1 = xtable(base_summary1, caption=c('Base case results summary.'),
                              label='tab:base_summary1',digits=0)
  # # Add alignment   
  align(base_summary.table1) = c('l',
                                'r',
                                '>{\\centering}p{1.1in}',
                                '>{\\centering}p{1.1in}',
                                '>{\\centering}p{1.1in}',
...
if (n_models > 1) {
  # Bind data from all three models together
  base_summary2 = as.data.frame(cbind(mngmt2,mod2_summary))
  
  cat("Got 2\n")
  
  
  # Transpose the dataframe to create the table and create data labels  
  base_summary2 = as.data.frame(t(base_summary2))
  base_summary2$names=c('Landings (mt)',
                       'Total Est. Catch (mt)',
                       'OFL (mt)', 
                       'ACL (mt)',
  ...
  base_summary2 = base_summary2[,c(ncol(base_summary2),
                                 (ncol(base_summary2)-1),
                                1:(ncol(base_summary2)-2))]
  
  colnames(base_summary2) = c('Quantity',seq(FirstYR+1,LastYR+1))
  
  # # Create the table
  base_summary.table2 = xtable(base_summary2, caption=c(paste(spp,' base case results summary.',sep='')),
                              label='tab:base_summary2',digits=0)
  # # Add alignment   
  align(base_summary.table2) = c('l',
                                'r',
                                'r',
                                '>{\\centering}p{1.1in}',
                                '>{\\centering}p{1.1in}',
...
if (n_models == 3) {
# Bind data from all three models together
base_summary3 = as.data.frame(cbind(mngmt3, mod3_summary))
    
cat("Got 3\n")

# Transpose the dataframe to create the table and create data labels  
base_summary3 = as.data.frame(t(base_summary3))

base_summary3$names=c('Landings (mt)',
                     'Total Est. Catch (mt)',
                     'OFL (mt)', 
                     'ACL (mt)',
...
base_summary3 = base_summary3[,c(ncol(base_summary3),
                              (ncol(base_summary3)-1),
                              1:(ncol(base_summary3)-2))]

colnames(base_summary3) = c('Quantity',seq(FirstYR+1,LastYR+1))



# Create the table
base_summary.table3 = xtable(base_summary3, caption=c('Base case results summary.'), 
                                  label='tab:base_summary3',digits=0) 
# Add alignment   
align(base_summary.table3) = c('l',
                              'r',
                              'r', 
                              '>{\\centering}p{1.1in}', 
                              '>{\\centering}p{1.1in}', 
  ...
  Tot.catch.df1 = as.data.frame(Tot.catch.df)
  colnames(Tot.catch.df1)<-c("Year", "Catch")
  
  ###########################################################################################################################  
  
  # Lowest four recruitment years 
  RecDevs.all = mod2$recruitpars[grep('Late_RecrDev_', rownames(mod2$recruitpars)), c("Value", "Parm_StDev")]
  ind = sort(RecDevs.all[, "Value"], index.return = TRUE)$ix[1:4]
  find.yr = rownames(mod2$recruitpars[grep('Late_RecrDev', rownames(mod2$recruitpars)), ])
  temp = substring(find.yr,14)
  recdev.lowest = temp[ind]
  
  # Lowest SB
  find.sb = mod2$derived_quants[grep('SSB', mod2$derived_quants$Label), ]
  temp = find.sb[find.sb$Label >= paste('SSB_', Dat_start_mod2, sep='') & find.sb$Label <= paste('SSB_', Dat_end_mod2,  sep=''), ]  
  ind = sort(temp$Value, index.return = TRUE)$ix[1]
  ssb.yr = substring(temp$Label, 5)
  low.ssb = ssb.yr[ind]
  
  low.dep.value = paste0( round(100*mod2$derived_quants[mod2$derived_quants$Label == paste0("SSB_", low.ssb), 'Value'] / 
                                  mod2$derived_quants[mod2$derived_quants$Label == "SSB_Virgin", 'Value'],1), "%")
  
  Tot.catch = aggregate(ret_bio ~ Yr, FUN = sum, mod2$catch)$ret_bio
  Tot.catch.df = cbind(Dat_start_mod2:Dat_end_mod2, Tot.catch)
  temp = sort(Tot.catch.df[,2], index.return = TRUE)$ix
  max.catch.5 = Tot.catch.df[(temp[length(temp)]-5):temp[length(temp)],]
  
  Tot.catch.df2 = as.data.frame(Tot.catch.df)
  colnames(Tot.catch.df2)<-c("Year", "Catch")
  
  ###################################################################################################################################
  # Lowest four recruitment years 
  RecDevs.all = mod3$recruitpars[grep('Late_RecrDev_', rownames(mod3$recruitpars)), c("Value", "Parm_StDev")]
  ind = sort(RecDevs.all[, "Value"], index.return = TRUE)$ix[1:4]
  find.yr = rownames(mod3$recruitpars[grep('Late_RecrDev', rownames(mod3$recruitpars)), ])
  temp = substring(find.yr,14)
  recdev.lowest = temp[ind]
  
  # Lowest SB
  find.sb = mod3$derived_quants[grep('SSB', mod3$derived_quants$Label), ]
  temp = find.sb[find.sb$Label >= paste('SSB_', Dat_start_mod3, sep='') & find.sb$Label <= paste('SSB_', Dat_end_mod3,  sep=''), ]  
  ind = sort(temp$Value, index.return = TRUE)$ix[1]
  ssb.yr = substring(temp$Label, 5)
  low.ssb = ssb.yr[ind]
  
  low.dep.value = paste0( round(100*mod3$derived_quants[mod3$derived_quants$Label == paste0("SSB_", low.ssb), 'Value'] / 
                                  mod3$derived_quants[mod3$derived_quants$Label == "SSB_Virgin", 'Value'],1), "%")
  
  Tot.catch = aggregate(ret_bio ~ Yr, FUN = sum, mod3$catch)$ret_bio
  Tot.catch.df = cbind(Dat_start_mod3:Dat_end_mod3, Tot.catch)
  temp = sort(Tot.catch.df[,2], index.return = TRUE)$ix
  max.catch.5 = Tot.catch.df[(temp[length(temp)]-5):temp[length(temp)],]
  
  Tot.catch.df3 = as.data.frame(Tot.catch.df)
  colnames(Tot.catch.df3)<-c("Year", "Catch")
